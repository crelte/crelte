...
The `craft` directory contains a standard Craft CMS installation running the newest version of Craft CMS. It is responsible for defining content structure, managing entries, and exposing content via GraphQL.
...
- `index.html` is the HTML entry point for the frontend.
  - The `<html>` tag must include the `lang="<!--page-lang-->"` attribute.
