...
- `<sectionHandle>-<entryType>.svelte`
- `<sectionHandle>.svelte` (if the section has only one entry type)
...
If no matching template is found, an error is being logged to the console.
...
:::tip Unknown error
When Craft CMS is unavailable or a Svelte error occurs, Crelte renders [`src/Error.svelte`](#error-svelte).
:::
...
`src/server.js`. If you wan't to have a specific status returned then replace the value `true`
with the status code, e.g. `503`.
