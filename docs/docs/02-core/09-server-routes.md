...
Server routes are HTTP request handlers which can be anything from an API endpoint to
...
basically a WebApi request with some added properties. The idea behind `CreateServerRequest`
is to be really similar to the main `Crelte` type allowing for the use of known methods like `query`.
